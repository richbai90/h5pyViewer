...
def MplAddColormap(m, lut):
    if type(lut) == dict:
        try:
            ...
            lstG = lut["green"]
            lstB = lut["blue"]
            kR, vR, dummy = list(zip(*lstR))
            kG, vG, dummy = list(zip(*lstG))
            kB, vB, dummy = list(zip(*lstB))
        ...
        kLst.update(kB)
        kLst = sorted(kLst)
        vRGB = list(zip(np.interp(kLst, kR, vR), np.interp(kLst, kG, vG), np.interp(kLst, kB, vB),))
        lut2 = list(zip(kLst, vRGB))
    else:
        if type(lut[0][1]) == tuple:
            lut2 = lut
        else:
            kLst = np.linspace(0.0, 1.0, num=len(lut))
            lut2 = list(zip(kLst, lut))
...
class GLCanvasImg(wx.glcanvas.GLCanvas):
    ...
    def OnPaint(self, event):
        ...
        self.glColBar.draw(-0.5, -0.5, 0, 1.0, 0.02)
...
"""
    ...
    def InitGL(self):
        ...
        txrColBar = np.linspace(0.0, 1.0, 256).astype(np.float32)
        self.glColBar = glumpy.image.Image(txrColBar, colormap=colMap, vmin=0, vmax=1)

        self.UpdateImg()
        self.imgCoord = np.array([-0.49, 0, 0.49, 1, -0.49, 0, 0.49, 1])  # xmin,xmax,umin,umax,ymin,ymax,vmin,vmax
        pass
