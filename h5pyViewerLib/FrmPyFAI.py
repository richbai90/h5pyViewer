...
class MPLCanvasPyFAI1D(FigureCanvas):
    ...
    def InitChild(self, data):
        ...
        self.ai = pyFAI.AzimuthalIntegrator(1.0e3, ctrX, ctrY, 0.0, 0.0, 0.0, 1.0e0, 1.0e0)
        # canvas=self.canvas
        self.numPtTh = int(np.average(data.shape) / 2.0)
...
class HdfPyFAIFrame(HdfImageGLFrame):
    def __init__(self, parent, title, hid):
        ...
        raw
        self.numPtTh = int(np.average(raw.shape) / 2.0)
        self.numPtCh = 360
...
if __name__ == "__main__":
    ...
    def GetParser(required=True):
        ...
        parser.add_argument(
            "--elem", required=required, default=elem, help="the path to the element in the hdf5 file",
        )
        return parser
