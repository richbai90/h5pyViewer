...
class AboutFrame(wx.Frame):
    def __init__(self, parent):
        ...
        bmp = wx.StaticBitmap(
            panel, -1, wx.Bitmap(os.path.join(imgDir, "splash1.png"), wx.BITMAP_TYPE_ANY), (30, st0.Position[1] + st0.Size[1] + 10),
        )
...
class HdfTreePopupMenu(wx.Menu):
    ...
    def OnShowAttrib(self, event):
        ...
        if type(hid) == tuple:
            hid = hid[0]  # external link->get dataset
    ...
    def OnShowRoiStat(self, event):
        ...
        dlg = wx.FileDialog(
            wxTree,
            "Choose ROI mask file (e.g. pilatus_integration_mask.mat)",
            os.getcwd(),
            "",
            "MATLAB files (*.mat)|*.mat|all (*.*)|*.*",
            wx.FD_OPEN | wx.FD_CHANGE_DIR,
...
class HdfViewerFrame(wx.Frame):
    ...
    def OnOpen(self, event):
        dlg = wx.FileDialog(
            self, "Choose a file", os.getcwd(), "", "HDF5 files (*.hdf5;*.h5)|*.hdf5;*.h5|all (*.*)|*.*", wx.FD_OPEN | wx.FD_CHANGE_DIR,
        )
        if dlg.ShowModal() == wx.ID_OK:
            path = dlg.GetPath()
